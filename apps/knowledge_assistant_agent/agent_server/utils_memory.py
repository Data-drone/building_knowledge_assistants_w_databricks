...
def _is_lakebase_hostname(value: str) -> bool:
    """Hostnames look like instance-{uuid}.database.{env}.cloud.databricks.com."""
...
def resolve_lakebase_instance_name(
    ...
    """Resolve a Lakebase hostname to the actual instance name via the SDK.

    If the input is already a plain instance name (not a hostname), it is
    returned unchanged.
    """
...
def get_lakebase_access_error_message(lakebase_desc: str) -> str:
    """Human-readable error message for Lakebase connectivity failures."""
    if os.getenv("DATABRICKS_APP_NAME"):
        app_name = os.getenv("DATABRICKS_APP_NAME")
        return (
            f"Failed to connect to Lakebase instance '{lakebase_desc}'. "
            ...
            "See the README section 'Grant Lakebase permissions' for the SQL commands."
        )
    return (
        f"Failed to connect to Lakebase instance '{lakebase_desc}'. "
        "Please verify:\n"
...
def memory_tools():
    """Factory returning three async tools for long-term user memory.

    Each tool reads ``user_id`` and ``store`` from
    ``config["configurable"]`` at call-time so the tools themselves are
    stateless and can be created once at graph-build time.
    """
    ...
    async def save_user_memory(
        ...
        Args:
            memory_key: A short, descriptive key for the memory (e.g. 'favorite_color').
